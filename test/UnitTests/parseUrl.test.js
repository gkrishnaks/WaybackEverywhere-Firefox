...
describe("URL Parser", function() {
  ...
  it("can return hostname when live url is passed to function", function() {
    var result = getHostfromUrl("https://gnu.org/manual/make").hostname; // my_add is definied in calculator.js
    expect(result).to.eql("gnu.org");
  });

  it("can return full url when live url is passed to function", function() {
    var result = getHostfromUrl("https://gnu.org/manual/make").url; // my_add is definied in calculator.js
  ...
  it("should return fully qualified urls if url is excluded", function() {
    ...
    expect(getHostfromUrl(urls[0]).url).to.eql("https://www.quora.com/");
    expect(getHostfromUrl(urls[1]).url).to.eql("http://www.quora.com/");
    expect(getHostfromUrl(urls[2]).url).to.eql("http://www.quora.com/");
    expect(getHostfromUrl(urls[3]).url).to.eql("http://quora.com/");
  });
});
